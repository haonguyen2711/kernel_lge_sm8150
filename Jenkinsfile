pipeline {
  ...
  environment {
    // Khai báo biến dùng toàn cục nếu cần
  ...
  stages {
    stage('Checkout') {
      steps {
        // Clone code từ GitHub (Jenkins dùng webhook nên sẽ tự động trigger)
        checkout scm
      }
    }
    ...
    stage('Build Kernel') {
      steps {
        sh '''
          ...
          export LD_LIBRARY_PATH=${CLANG_PATH/\\/bin/\\/lib64}:$LD_LIBRARY_PATH
    ...
    // (Tuỳ chọn) Có thể thêm stage để upload lên GitHub Release nếu cần
  }
